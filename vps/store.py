...
class LocalStore:
    ...
    def initialize(self) -> None:
        ...
        with self._lock, self._connect() as db:
            ...
            columns = {row["name"] for row in db.execute("PRAGMA table_info(thirdparty_subs)").fetchall()}
    ...
    def set_runtime(self, slot_id: str, **values: Any) -> ExitSlotSnapshot:
        ...
        parameters.extend((int(time.time()), slot_id))
        with self._lock, self._connect() as db:
    ...
    def add_vps(self, ip: str, name: str = "", os_type: str = "debian") -> dict[str, Any]:
        with self._lock, self._connect() as db:
            db.execute(
                "INSERT INTO vps_list (ip, name, os, created_at) VALUES (?, ?, ?, ?)",
                (ip, name, os_type, int(time.time())),
            )
    ...
    def update_vps(self, ip: str, **fields: Any) -> dict[str, Any]:
        allowed = {"name", "os", "egress_mode", "socks5_addr", "socks5_port", "socks5_user", "socks5_pass"}
    ...
    def list_nodes(self) -> list[dict[str, Any]]:
        with self._lock, self._connect() as db:
            rows = db.execute("SELECT * FROM node_list ORDER BY created_at").fetchall()
        return [dict(row) for row in rows]

    def add_node(self, ip: str, name: str = "", protocol: str = "", traffic_limit: int = 0, expire_time: int = 0) -> dict[str, Any]:
        with self._lock, self._connect() as db:
            cursor = db.execute(
                "INSERT INTO node_list (ip, name, protocol, traffic_limit, expire_time, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                (ip, name, protocol, traffic_limit, expire_time, int(time.time())),
            )
    ...
    def update_node(self, node_id: int, **fields: Any) -> dict[str, Any]:
        allowed = {"ip", "name", "protocol", "enable", "traffic_used", "traffic_limit", "expire_time"}
        unknown = set(fields) - allowed
        if unknown:
